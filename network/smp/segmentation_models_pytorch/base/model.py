...
D = TypeVar("D", bound="DepthModel")
L = TypeVar("L", bound="LanesModel")
...
class DepthModel(torch.nn.Module, SMPHubMixin):
    """Base class for all depth models."""
    ...
    # Fix type-hint for models, to avoid HubMixin signature
    def __new__(cls: Type[D], *args, **kwargs) -> D:
        instance = super().__new__(cls, *args, **kwargs)
        return instance
    ...
    def initialize(self):
        init.initialize_decoder(self.decoder)
        init.initialize_head(self.depth_head)
        if self.classification_head is not None:
            init.initialize_head(self.classification_head)
    ...
    def forward(self, x):
        ...
        masks = self.depth_head(decoder_output)
    ...
    def freeze_encoder(self):
        ...
        - Puts normalization layers that track running statistics
            ...
            so their ``running_mean`` and ``running_var`` are no longer updated.
        """
        print("[DepthModel] Encoder frozen.")
        return self._set_encoder_trainable(False)
...
# -------------------------
    ...
    def load_from_segmentation_checkpoint(
        ...
        print(f"[DepthModel] Loading from segmentation checkpoint: {ckpt_path}")
        ckpt = torch.load(ckpt_path, map_location="cpu")
        state = ckpt["model_state"] if isinstance(ckpt, dict) and "model_state" in ckpt else ckpt

        if load_encoder:
            enc = {k.replace("encoder.", ""): v for k, v in state.items() if k.startswith("encoder.")}
            ret = self.encoder.load_state_dict(enc, strict=strict)
            ...
            print("  [encoder] NOT loaded (Kaiming init)")

        if load_decoder:
            dec = {k.replace("decoder.", ""): v for k, v in state.items() if k.startswith("decoder.")}
            ret = self.decoder.load_state_dict(dec, strict=strict)
            if not strict and ret is not None:
                missing, unexpected = ret
            ...
            print("  [decoder] NOT loaded (Kaiming init)")
    ...
    def freeze_decoder(self):
        ...
        print("[DepthModel] decoder frozen.")
        return self
...
class LanesModel(torch.nn.Module, SMPHubMixin):
    """Base class for all lane detection models."""
    ...
    # Fix type-hint for models, to avoid HubMixin signature
    def __new__(cls: Type[L], *args, **kwargs) -> L:
        instance = super().__new__(cls, *args, **kwargs)
        return instance
    ...
    def initialize(self):
        init.initialize_decoder(self.decoder)
        init.initialize_head(self.lanes_head)
        if self.classification_head is not None:
            init.initialize_head(self.classification_head)
    ...
    def forward(self, x):
        ...
        masks = self.lanes_head(decoder_output)
...
# -------------------------
    def load_from_segmentation_checkpoint(
        ...
        encoder_partial_depth: int | None = None,
    ):
        print(f"[LanesModel] Loading from segmentation checkpoint: {ckpt_path}")
        ckpt = torch.load(ckpt_path, map_location="cpu")
        state = ckpt["model_state"] if isinstance(ckpt, dict) and "model_state" in ckpt else ckpt

        # -------------------------------------------------
        # Encoder loading (full or partial)
        # -------------------------------------------------
        if load_encoder:
            enc_state = {
                k.replace("encoder.", ""): v
            ...
            # Track which params we are going to load
            self._encoder_loaded_param_names = set(enc_state.keys())

            if encoder_partial_load:
                if encoder_partial_depth is None:
                    raise ValueError(
                        "encoder_partial_depth must be specified when encoder_partial_load=True"
                    )

                print(
                    f"[encoder] PARTIAL loading enabled | depth <= {encoder_partial_depth}"
                )

                enc_state = self._filter_encoder_state_by_depth(
                    enc_state,
                    max_depth=encoder_partial_depth,
                )

                # Update loaded keys after filtering
                self._encoder_loaded_param_names = set(enc_state.keys())

            ret = self.encoder.load_state_dict(enc_state, strict=False)

            if ret is not None:
                missing, unexpected = ret
                if missing:
                    print("  [encoder] missing keys:", len(missing))
                if unexpected:
                    print("  [encoder] unexpected keys:", len(unexpected))

            print(
                f"  [encoder] loaded ({'partial' if encoder_partial_load else 'full'}) | "
                f"params={len(self._encoder_loaded_param_names)}"
            )
        else:
            print("  [encoder] NOT loaded (Kaiming init)")
            self._encoder_loaded_param_names = set()
        ...
        if load_decoder:
            ...
            ret = self.decoder.load_state_dict(dec_state, strict=strict)
    ...
    def _filter_encoder_state_by_depth(
        self,
        enc_state: dict[str, torch.Tensor],
        max_depth: int,
    ) -> dict[str, torch.Tensor]:
        """
        Filter encoder state_dict to keep only parameters belonging
        to stages <= max_depth.

        This is heuristic-based and works for most timm/SMP encoders.
        """
        ...
        for k, v in enc_state.items():
            ...
            #   features.3.*
            #
            # We extract the first integer index and compare.

            match = re.search(r"(blocks|layers|layer|stages|features)\.(\d+)", k)
            ...
            if match:
                ...
                # Always keep stem / initial convs
                if k.startswith(("conv", "bn", "stem", "patch_embed", "downsample")):
                    keep = True
                else:
                    # If no index found, be conservative: keep
                    keep = True
        ...
        print(
            f"[encoder] partial filter: kept={kept} | skipped={skipped} | max_depth={max_depth}"
        )
    ...
    def freeze_decoder(self):
        ...
        print("[LanesModel] decoder frozen.")
        return self
