...
from training.segmentation.trainer_base import SegmentationTrainerBase
from network.smp.DeepLabv3Plus import DeepLabV3Plus



class DeepLabV3PlusTrainer(SegmentationTrainerBase):
    """
    ...
    # Construction
    # -------------------------
    def __init__(self, cfg: dict, is_training: bool = True):
        ...
        self._build_training_state() if self.is_training else None

        #resume in case the checkpoint path is specified
        self._maybe_resume() if self.is_training else None
    ...
    def _build_model_stack(self):
        ...
        if network_model == "deeplabv3plus":
            ...
            #custom implementation of DeeplabV3Plus for segmentation
            self.model = DeepLabV3Plus(
                ...
                #head params
                head_upsampling=self.head_cfg.get("head_upsampling", 4),
                ...
                output_channels=self.network_cfg.get("output_channels", 19),    #number of segmentation classes
                )
        ...
        #build rest of the model stack: optimizer and scheduler in case we are not doing evaluation
